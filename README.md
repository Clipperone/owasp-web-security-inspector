...
- The exported report is emitted with a stable `schemaVersion` for CI/issue-template reuse. A **posture bar** (High · Medium · Low · Info) and per-finding **filters** (minimum severity, "actionable only", text search) sit atop the Assessment tab; the same filters drive both the view and the export, so the counts always match. Reports can be **copied** or **downloaded** (`owasp-assessment-<host>-<timestamp>.md/.json`) in Markdown or JSON.
...
- Storage scanning covers `localStorage`, `sessionStorage`, and **IndexedDB** (where many modern apps keep tokens). JWT checks include a not-yet-valid (`nbf`) signal, and cross-site (`SameSite=None`) cookies without the **Partitioned** attribute (CHIPS) are flagged.
